...
#   mutate(
#     hcc_log_m = if_else(
#       hcc_log_m > mean(hcc_log_m, na.rm = TRUE) + 3 * sd(hcc_log_m, na.rm = TRUE) |
#         hcc_log_m < mean(hcc_log_m, na.rm = TRUE) - 3 * sd(hcc_log_m, na.rm = TRUE),
#     NA_real_, hcc_log_m
#     ),
#     hcc_log_i = if_else(
#       hcc_log_i > mean(hcc_log_i, na.rm = TRUE) + 3 * sd(hcc_log_i, na.rm = TRUE) |
#         hcc_log_i < mean(hcc_log_i, na.rm = TRUE) - 3 * sd(hcc_log_i, na.rm = TRUE),
#     NA_real_, hcc_log_i
#     )
#   ) %>%
...
# Counts across phases
```{r}
hc %>% 
  filter(!is.na(hcc_log_m), ID >= 1000) %>% 
  count(phase)
```

# Distribution of gestational weeks
```{r}
hc %>% 
  group_by(timepoint) %>% 
  summarise_at(
...
# Trajectory of maternal HCC
```{r}
hc %>% 
  filter(segment < 4 & ID >= 1000) %>% 
  ggplot(aes(seg_week, hcc_log_m)) +
  geom_smooth(color = "black", span = 1, size = 3) +
  geom_vline(
    xintercept = mean_gestionational_weeks_birth, 
    color = "red", 
    linetype = "dashed",
    size = 2
  ) + 
  geom_label(
    x = mean_gestionational_weeks_birth, 
    y = .35,
    size = 5,
    label = "Mean gestational weeks\nat birth"
  ) +
  scale_x_continuous(breaks = seq.int(from = 0, to = 70, by = 10)) +
  scale_y_continuous(breaks = seq.int(from = .5, 2, .2)) +
  labs(
    x = "Weeks since conception",
    y = "Maternal log(HCC) pg/ml"
  ) +
  theme_hcc
...
hc %>% 
  ggplot(aes(seg_week, hcc_log_m)) +
  geom_point(aes(color = phase), size = 3, alpha = 1/2) +
  geom_smooth(color = "black", se = FALSE, size = 3) +
  theme_minimal() +
  theme(
    panel.grid = element_blank(),
    plot.title = element_text(size = 18, hjust = .5),
...
hc %>% 
  ggplot(aes(seg_week, hcc_log_m)) +
  geom_smooth(color = "black", se = FALSE, size = 1, span = 1) +
  theme(
    panel.grid = element_blank(),
    plot.title = element_text(size = 18, hjust = .5),
    axis.title = element_text(size = 18),
...
## Visualize and test dyadic concordance
```{r}
hc_t3 %>% 
  ggplot(aes(x = hcc_m_mean_t2, y = hcc_log_i)) +
  geom_point() +
  geom_smooth(method = "lm") +
  theme_minimal() +
  labs(
    x = "Maternal log(HCC) 6 months postpartum",
    y = "Infant log(HCC) at age 6 months"
  )

cor.test(hc_t3$hcc_log_i, hc_t3$hcc_m_mean_t2)
```
